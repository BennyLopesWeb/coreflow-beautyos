...
| R4-F6+ | Migrar `Payment`/`Schedule` legado para modelos core dedicados (pré-requisito físico), então drop físico de `agendamentos`/`payments`/`schedules` legado (separado ADR), após confirmação de que não há mais consumidores lendo dados legado (sync services, reconciliation, relatórios) |
