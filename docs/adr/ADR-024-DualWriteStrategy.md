...
| R4-F5 | ✅ **FK `booking_id` + fechamento do gap operacional** — `queue_entries`/`fila` ganham FK nullable indexada para `core_bookings.id` (migration `cf013_r4_f5_booking_id`); `checkin`/`iniciar`/`concluir` avançam `CoreBooking.status` para entradas core-only; admin de pagamentos legado marcado `deprecated=True`. Ainda sem DROP; `2.8.0-r4-f5` |
| R4-F6 | ✅ **Bridge `Payment`→`booking_id` + disponibilidade core-only + 410 admin legado** — `payments.agendamento_id` nullable + `payments.booking_id` (migration `cf014_r4_f6_payment_booking_id`); `confirmar_deposito_por_booking` cria/atualiza `Payment` ponte; `DisponibilidadeService` expira `CoreBooking` pendente via `CancelBookingHandler`; `aceitar_reagendamento` para de escrever `Schedule`; `POST /admin/pagamentos/{agendamento_id}/confirmar-sinal` → `410 Gone`. Ainda sem DROP — `Schedule`/`SatisfactionSurvey` mantêm FK física para `agendamentos`; `2.9.0-r4-f6` |
| R4-F7 | ✅ **Decouple físico das FKs restantes** — FK física para `agendamentos.id` removida de `payments`, `schedules`, `satisfaction_surveys`, `fila`, `queue_entries`, `financeiro` e `notification_logs` (migration `cf015_r4_f7_decouple_fks`); `schedules`/`satisfaction_surveys` ganham `booking_id` (FK nullable `core_bookings.id`) + `agendamento_id` nullable; `DisponibilidadeService._slots_ocupados` passa a ler ocupação exclusivamente de `core_bookings` (leitura de compatibilidade sobre `Agendamento` removida). Nenhuma tabela referencia mais `agendamentos` por constraint física — tabela mantida (sem DROP) para fixtures/histórico; `2.10.0-r4-f7` |
| R4-F8 | ✅ **DROP físico de `agendamentos`** — migration `cf016_r4_f8_drop_agendamentos` executa `DROP TABLE agendamentos` (idempotente); `app/models/agendamento.py`: `Agendamento` deixa de herdar `Base` (não mapeado — `create_all` não recria a tabela); enums preservados; toda leitura/escrita de produção sobre `Agendamento` reescrita para no-op (`NotFoundError`/vazio) ou `CoreBooking` equivalente. `payments`/`schedules`/`fila`/`queue_entries`/`financeiro`/`notification_logs` mantêm a coluna `agendamento_id` como `Integer` histórico (sem DROP dessas tabelas/colunas nesta release); `2.11.0-r4-f8` — **encerra o Strangler Fig `Agendamento`↔`CoreBooking`** |
