...
| R4-F7 | ✅ **Decouple físico das FKs restantes** — FK física para `agendamentos.id` removida de `payments`, `schedules`, `satisfaction_surveys`, `fila`, `queue_entries`, `financeiro` e `notification_logs` (migration `cf015_r4_f7_decouple_agendamento_fks`); `schedules`/`satisfaction_surveys` ganham `booking_id` (FK nullable `core_bookings.id`) + `agendamento_id` nullable; `DisponibilidadeService._slots_ocupados` passa a ler ocupação exclusivamente de `core_bookings` (leitura de compatibilidade sobre `Agendamento` removida). Nenhuma tabela referencia mais `agendamentos` por constraint física — tabela mantida (sem DROP) para fixtures/histórico; `2.10.0-r4-f7` |
| R4-F8 | DROP físico de `agendamentos`/`payments`/`schedules` legado (separado ADR), condicionado à migração/arquivamento de dados históricos e confirmação de que não há mais consumidores lendo dados legado (sync services, reconciliation, relatórios, `AdminService.listar_pagamentos`) |
