...
| R4-F7+ | DROP físico de `agendamentos`/`payments`/`schedules` legado (separado ADR), após remover as últimas FKs físicas dependentes (`Schedule.agendamento_id`, `SatisfactionSurvey.agendamento_id`) e confirmação de que não há mais consumidores lendo dados legado (sync services, reconciliation, relatórios, `AdminService.listar_pagamentos`) |
