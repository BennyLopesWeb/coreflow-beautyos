"""
Command MarkNoShowBooking — CQRS CoreFlow (R4-F13 / ADR-026).

Transição ``approved → no_show``.
"""
...
class MarkNoShowBookingCommand:
    """
    Comando para marcar no-show.
...
class MarkNoShowBookingHandler:
    """
    Handler CQRS — no_show core-only (R4-F13).
    ...
    def execute(self, command: MarkNoShowBookingCommand) -> CoreBooking:
        """
        Executa mark no-show.
        ...
        if not feature_flags.is_enabled("booking.core.enabled"):
            raise BusinessRuleError(
                "No-show exige FEATURE_BOOKING_CORE_ENABLED=true."
            )
        ...
        try:
            try:
                booking = domain_service.mark_no_show(booking, command.reason)
            ...
            from app.modules.booking.domain.events import booking_no_show
