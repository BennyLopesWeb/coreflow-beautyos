"""
CatalogQueryAdapter — leitura catalog/offering via ORM (F1 interim).
"""
...
class SqlAlchemyCatalogQueryAdapter:
    """
    Implementação read-only de ``CatalogQueryPort``.
    ...
    def get_offering_snapshot(
        ...
        catalog = (
            ...
            .filter(CoreCatalog.id == catalog_id, CoreCatalog.company_id == company_id)
            .first()
        )
        offering = (
            self._db.query(CoreOffering)
            .filter(CoreOffering.id == offering_id, CoreOffering.company_id == company_id)
            .first()
        )
